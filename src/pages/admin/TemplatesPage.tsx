...
export default function AdminTemplatesPage() {
  ...
  const fetchTemplates = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await api<MessageTemplate[]>('/api/admin/templates');
      setTemplates(data);
    } catch (error) {
      toast.error("Failed to fetch global templates.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const onSubmit = async (values: TemplateFormValues) => {
    const apiCall = selectedTemplate
      ? api(`/api/admin/templates/${selectedTemplate.id}`, { method: 'PUT', body: JSON.stringify(values) })
      : api('/api/admin/templates', { method: 'POST', body: JSON.stringify(values) });
    try {
      await apiCall;
  ...
  const handleDelete = async () => {
    ...
    try {
      await api(`/api/admin/templates/${selectedTemplate.id}`, { method: 'DELETE' });
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen} className="sm:max-w-[625px]">
        <DialogContent className="sm:max-w-[625px]">
          <DialogHeader>
            ...
            <DialogDescription>Design the message for WhatsApp and plain text.</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel>Template Name</FormLabel><FormControl><Input placeholder="e.g., Formal Invitation" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="content_wa" render={({ field }) => (<FormItem><FormLabel>WhatsApp Content</FormLabel><FormControl><Textarea placeholder="Use [nama-tamu] and [link-undangan] as placeholders." {...field} rows={8} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="content_copy" render={({ field }) => (<FormItem><FormLabel>Plain Text Content</FormLabel><FormControl><Textarea placeholder="Content for copying to clipboard." {...field} rows={8} /></FormControl><FormMessage /></FormItem>)} />
              <DialogFooter>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete the "{selectedTemplate?.name}" template.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
